...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert lower_bound(y) * upper_bound(y) > 0, 'interval y can not span zero.'
...
def quadratic(x, a, b, c):
    ...
    ex_point = -b / (2 * a)
    bound = [f(lower), f(upper)]
    if ex_point > lower and ex_point < upper:
        bound += [f(ex_point)]

    return interval(min(bound), max(bound))
