...
class VendingMachine:
    ...
    def vend(self):
        if self.stock == 0:
            ...
            self.balance = 0
            res = 'Here is your {0}'.format(self.product)
            if change:
                res += ' and ${0} change'.format(change)
            return res + '.'
...
def store_digits(n):
    ...
    #     while n // mask >= 10:
    #         mask *= 10
    #     first_digit, rest_digits = n // mask, n - first_digit * mask
...
def is_bst(t):
    ...
    if length > 2:
        ...
        bs = t.branches
        return is_bst(bs[0]) and is_bst(bs[1]) and bst_max(bs[0]) <= t.label and bst_min(bs[1]) > t.label
