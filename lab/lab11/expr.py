...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        """
        "*** YOUR CODE HERE ***"
        operator, operands = self.operator.eval(env), list(map(lambda o: o.eval(env), self.operands))
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        "*** YOUR CODE HERE ***"
        self.current = self.parent.copy()
        self.current.update(zip(self.parameters, arguments))
        return self.body.eval(self.current)
