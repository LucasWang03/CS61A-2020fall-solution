...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        args = expr.rest.map(lambda e: scheme_eval(e, env))
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 9
        "*** YOUR CODE HERE ***"
        symbol, formals, body = target.first, target.rest, expressions.rest
        func = do_lambda_form(Pair(formals, body), env)
        env.bindings[symbol] = func
        return symbol
        # END PROBLEM 9
    else:
...
def optimize_tail_calls(original_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        # BEGIN PROBLEM 19
        "*** YOUR CODE HERE ***"
        # tail call 和 macro这两题我看了半天连题目都看不懂，只能作罢
        # 后来才知道要先上完后面两节optional lecture才能做，不过我也懒得做了，各位想做的可以看看其他的解答
        # END PROBLEM 19
